...
class BackwardCompatibilityTest extends PHPUnit_Framework_TestCase
{
    protected function setUp()
    {
        ...
        if (!class_exists('OpenGraphProtocol', false)) {
            ...
            set_error_handler(function ($errno, $errstr, $errfile, $errline) use ($testObject, &$errorCount) {
                $testObject->assertEquals(E_USER_DEPRECATED, $errno);
                $testObject->assertEquals('Please configure NiallKennedy\\OpenGraphProtocolTools with your autoloader', $errstr);
                $errorCount++;
            });
            require $packageRoot . DIRECTORY_SEPARATOR . 'media.php';
    ...
    {
        $this->markTestSkipped('known broken test... need to fix.' );
